...
def apply_column_type_changes(df, changes, date_col=None):
    ...
    num = list(set(num + new_num))
    cat = list(set(cat + new_cat))
    if date_col:
...
def explore_data(df, date_col=None, num=None, cat=None, dtp="train"):
    ...
    if num:
        ...
        sns.heatmap(df[num].corr(), annot=True, fmt=".2f", linewidths=0.5, ax=ax)
        st.pyplot(fig)
        vals = [c for c in num if pd.api.types.is_numeric_dtype(df[c])]
        if vals:
            st.markdown("**Outliers**")
            fig, axes = plt.subplots(1, len(vals), figsize=(4*len(vals),4))
            axes = [axes] if len(vals)==1 else axes
...
def engineer_features(train,test,num,cat,target='sales'):
    ...
    if target in tr and 'onpromotion' in tr:
        tr['sales_onpromo'] = tr[target]*tr['onpromotion']; te['sales_onpromo'] = te['onpromotion']*0 if 'onpromotion' in te else 0
    if 'onpromotion' in tr and 'is_weekend' in tr:
        tr['promo_weekend'] = tr['onpromotion']*tr['is_weekend']; te['promo_weekend'] = te['onpromotion']*te['is_weekend'] if 'onpromotion' in te and 'is_weekend' in te else 0
    for col in ['city','state']:
        if col in tr:
            m = tr.groupby(col)[target].mean().to_dict(); tr[f'{col}_encoded']=tr[col].map(m); te[f'{col}_encoded']=te[col].map(m).fillna(tr[target].mean())
    for col in ['family','locale_name']:
        if col in tr:
            f = tr[col].value_counts(normalize=True).to_dict(); tr[f'{col}_encoded']=tr[col].map(f); te[f'{col}_encoded']=te[col].map(f).fillna(0)
    ...
    for df in [tr, te]:
        ...
        for c in ['transferred','is_weekend','is_holiday']: df[c] = df[c].astype(int) if c in df else df[c]
        ...
        if uf:
            ...
            if df is not None:
                ...
                with st.form("train_cfg"):
                    dc = st.selectbox("Date Column", ['None'] + list(df.columns))
                    dc = None if dc == 'None' else dc
                    ...
                    om = st.selectbox("Outliers", ['None', 'Remove', 'Replace'], key="train_o")
                    om = om.lower() if om != 'None' else None
                    nz = st.checkbox("Normalize")
                    st.form_submit_button("Apply", on_click=lambda: st.session_state.update({
                        'train_date_col': dc,
                        'train_target_col': tc,
                        'train_numeric_cols': num,
                        'train_categorical_cols': cat,
                        'train_outlier_method': om,
                        'train_normalize': nz
                    }))
                with st.form("train_proc"):
                    st.markdown("**Process Train Data**")
                    if st.form_submit_button("Run"):
                        df2 = apply_column_type_changes(
                            st.session_state['train_df'],
                            st.session_state.get('train_type_changes', {}),
                            st.session_state.get('train_date_col', None)
                        )[0]
                        explore_data(
                            df2,
                            st.session_state.get('train_date_col', None),
                        ...
                        proc, rem = preprocess_data(
                            df2,
                            st.session_state.get('train_numeric_cols', []),
                            st.session_state.get('train_categorical_cols', []),
                            st.session_state.get('train_date_col', None),
                            st.session_state.get('train_outlier_method', None),
                            st.session_state.get('train_normalize', False)
                        )
                        st.session_state['processed_train'] = proc
                        st.write(f"Processed: {rem} duplicates removed, {proc.shape[0]} rows remain")
                        st.dataframe(proc.head(), height=150)
    with tt_tab:
        uf2 = st.file_uploader("Upload Test Data", type=['csv','parquet'], key="test")
        if uf2:
            df2 = load_data(uf2, "test")
            if df2 is not None:
                st.session_state['test_df'] = df2
                st.dataframe(df2.head(), height=150)
                with st.form("test_cfg"):
                    dc2 = st.selectbox("Date Column", ['None'] + list(df2.columns), key="test_dc")
                    dc2 = None if dc2 == 'None' else dc2
                    num2, cat2 = detect_column_types(df2, dc2)
                    om2 = st.selectbox("Outliers", ['None', 'Remove', 'Replace'], key="test_o")
                    om2 = om2.lower() if om2 != 'None' else None
                    nz2 = st.checkbox("Normalize", key="test_nz")
                    st.form_submit_button("Apply", on_click=lambda: st.session_state.update({
                        'test_date_col': dc2,
                        'test_numeric_cols': num2,
                        'test_categorical_cols': cat2,
                        'test_outlier_method': om2,
                        'test_normalize': nz2
                    }))
                with st.form("test_proc"):
                    st.markdown("**Process Test Data**")
                    if st.form_submit_button("Run Test"):
                        df_clean = apply_column_type_changes(
                            st.session_state['test_df'],
                            st.session_state.get('test_type_changes', {}),
                            st.session_state.get('test_date_col', None)
                        )[0]
                        explore_data(
                            df_clean,
                            st.session_state.get('test_date_col', None),
                            st.session_state.get('test_numeric_cols', []),
                            st.session_state.get('test_categorical_cols', []),
                            "test"
                        )
                        proc2, rem2 = preprocess_data(
                            df_clean,
                            st.session_state.get('test_numeric_cols', []),
                            st.session_state.get('test_categorical_cols', []),
                            st.session_state.get('test_date_col', None),
                            st.session_state.get('test_outlier_method', None),
                            st.session_state.get('test_normalize', False),
                            "test"
                        )
                        st.session_state['processed_test'] = proc2
                        st.write(f"Processed Test: {rem2} duplicates removed, {proc2.shape[0]} rows remain")
                        st.dataframe(proc2.head(), height=150)
    st.divider()
    st.markdown("**Created with Belal Khamis**")
